...
- **Decision 2 (b) — eval-debt row** for Elder Care's lost `no_citation_source` provenance, `closes_when` the exclusion machinery is company-scoped. Opened by **T11**.
- **Decision 3 (b) — company-scope `gold_exclusions.yaml` now.** This is a new design fork, not a mechanical audit fix, so it is carried as its own subtask **T13** with its own architectural decision log rather than folded into T11 — and it is a **hub contract-surface extension** (gold corpora + gold config) that charter §3 M4's non-goals do not grant, so **A5 must grant it explicitly**. T13 cannot start before A5 lands; that is its first kill criterion.
- **Decision 4 — F-03 as registry row + eval-debt rows + operator disposition signoff**, matching Elder Care's `GAP-103` / `T12-bench-disposition` precedent. The pilot baseline `baseline_7174e0399e29` **stays**, labeled uninterpretable per-intent for the 12 bloated intents. No fresh Tier-3 escalation. Executed by **T11**.

**Carryability, stated honestly.** Adding T13 puts the distinct-slot count at 12 against a §Budget ceiling of 10 (T1–T9, one T10 amendment unit, T11, T12, T13). v1.5 argued the amendment-round exception for T11/T12; **T13 does not fit that exception** — §7 says a fix that introduces a new design choice is a re-plan, not an amendment, and company-scoped exclusions is exactly that. It is carried here only because the operator's Decision 3(b) is itself the Tier-2 ruling that authorizes it, and A5 will record the grant. Per §Budget this plan therefore emits a **split-proposal advisory** to the charter rather than silently absorbing the scope: if T13's kill criterion 2 fires (i.e. closing the disjointness invariant also requires company-scoping `kpi_claim_intent_map.yaml`), the structural work stops here and moves to M5 as its own charter-governed slice. See Validation record item 21.

**The four residual operator confirmations are answered** (`escalations/ESC-M4-1-rb-defect-t9-1-gold-corpus.md` §6, 2026-08-14), and every conditional branch in this plan is now resolved to a single live path:

- **A5 landed** — charter **v0.1.4, Amendment A5**, orchestrator-drafted and operator-ratified. It carries Decision 1's retroactive/non-precedential clause and a **bounded** surface grant: `gold_exclusions.yaml` plus `gold_labels/elder_care.yaml` (restoration write only); `kpi_claim_intent_map.yaml`, `clearsulting.yaml`, the `GoldLabel` schema and the `exclude_reason` vocabulary stay out of M4.
- **Restoration is in scope.** T13 restores Elder Care's three annotations, so **F-04 closes in T13 by restored truth** and T11's conditional F-04 output is **omitted** — exactly one subtask owns the finding.
- **The debt row opens in T11 and closes in T13**, same round, via `close_debt` with `closed_evidence_refs`; `open_debt_high_water_mark` is monotonic across the two commits.
- **F-03's disposition signoff is orchestrator-drafted, operator-signed**, and the baseline label lands on three surfaces: the registry row (tracked), a `signoffs/T9-clearsulting-pilot.md` addendum, and a caveat in the runbook's baseline-promotion policy section.

**All three remediation packets are emitted** — `packets/T11.md`, `packets/T12.md`, `packets/T13.md`. Execution order: T12 and T11 may run in parallel (see the runbook merge strategy at §5.4 item 14), T13 after T11.

**v1.5 amendment summary (audit remediation).** The audit's 8 majors cluster into three classes, routed per §7's own table:
(1) **F-01, F-02** — RB-DEFECT-T9-1 (`0a5b2f2`) reached into Elder Care's committed gold and a company-agnostic config
file with no governance record; §7's own routing sends gold-corpus changes to **Tier 2**, not a plain amendment. Routed
to escalation **ESC-M4-1** (`.dev/plans/eval-consolidation-m4-onboarding-runbook/escalations/ESC-M4-1-rb-defect-t9-1-gold-corpus.md`),
**pending operator ruling** — this is not the orchestrator's decision to make unilaterally. (2) **F-03** — the pilot's
bloated `filename_closure` intents need an operator disposition matching Elder Care's precedent (`GAP-103`); also
pending operator ruling, folded into **T11**, which is itself blocked on ESC-M4-1 because F-04's registry-rationale
fix touches the same row. (3) **F-05, F-06, F-07, F-08** (+ minors F-09, F-10, F-13) — coverage gaps and one
contract-string drift, none requiring an operator decision; specified now as **T12**, unblocked, ready to execute.
No re-plan: this is a §7-shaped remediation round with one Tier-2 escalation carved out, per the routing the plan
already committed to. Full detail at §7.
**Subtask count:** 13 (T1–T9 original + T10 amendment unit [T10-bis continuation, not an 11th slot, per Validation record item 19] + **T11, T12 audit-remediation amendment units + T13 structural fix, added v1.6 under ESC-M4-1 Decision 3(b)**) — declared §Budget 4–10 — **carryability check: exceeds ceiling by 2.** Per §Budget rule this would ordinarily require a HALT-and-split-proposal to the charter. It is *not* triggered here because T11/T12 are **amendment subtasks** responding to a post-Complete audit (§7), not new decomposition of the original task — the same class of exception the plan already used for T10/T10-bis (Validation record item 19's own "third functionally distinct defect would consume a slot" test does not apply to audit remediation, which §7 governs directly). Flagged for the charter's own records regardless: if a **second** post-audit remediation round is needed after T11/T12, that would be the signal to treat this milestone's amendment load as exceeding what one orchestrator plan should carry, and split is the right call at that point.
...
| `merge_exemption_companies_into_domain` `display_name` via `display_name_from_company_slug` (inverse paired with `canonical_company_slug` forward fold) | T12 / F-10 | `eval/retrieval/trust_statement.py::display_name_from_company_slug` | `eval/retrieval/tests/test_trust_statement.py::test_display_name_from_slug_round_trips_domain_companies` |
...
| **(v1.6, T13) Company-scoped gold exclusions** — `load_gold_exclusions(path: Path = GOLD_EXCLUSIONS_PATH, *, company_slug: str) -> dict[str, str]`: the exclusion artifact becomes keyed by company, and the loader **requires** a folded company slug and returns only that company's `intent_id → exclude_reason` mapping. A slug with no block returns `{}` (a company with no exclusions is legal); an unknown top-level shape raises `PreconditionError` as today. The slug is not re-folded here — callers pass a slug already produced by `companies.canonical_company_slug` (§2.2 write-path rule). | T13 | `eval/retrieval/gold/bootstrap.py::load_gold_exclusions` (keyword-only `company_slug`, no default) + `GoldLabelBootstrap._gold_exclusions()` keying its cache on the instance's company slug | `eval/retrieval/tests/test_gold_exclusions.py::test_load_gold_exclusions_requires_company_slug`, `…::test_exclusions_are_company_scoped_and_do_not_leak_across_companies`, `…::test_unknown_company_returns_empty_exclusions` |
| **(v1.6, T13) Exclusion artifact shape** — `eval/retrieval/gold/gold_exclusions.yaml` top level becomes `companies: {<canonical_slug>: {excluded: [{intent_id, exclude_reason}]}}`. `exclude_reason` stays inside the existing closed `EXCLUDE_REASON_VOCABULARY`; **no new field, no new reason value, no `GoldLabel` field change** — the company dimension is added to the *config file*, not to the gold row schema (§1 non-goal: rows not schema). | T13 | committed YAML artifact, loaded only through `load_gold_exclusions` | `…::test_committed_exclusions_artifact_validates`, plus the reshaped `test_gold_exclusions_totality_and_disjointness` (see T13 Outputs — the invariant becomes per-company) |
...
python -m eval.retrieval.harness_cli run --company-name "<Display Name>" --catalog uc13_ale --store-backend delta --run-type baseline [--gold-path <path>]
...
graph TD
    ...
    ESCM41[ESC-M4-1 Tier-2 escalation — RULED 2026-08-14]
    T11[T11 amendment: registry & gold-corpus honesty remediation]
    T12[T12 amendment: coverage-gap remediation]
    A5[Charter Amendment A5 — ESC-M4-1 ruled, promotion pending]
    T13[T13 amendment: company-scoped gold exclusions]
...
**v1.6 amendment edges (ESC-M4-1 ruled).** `ESCM41 --> A5`: the ruling is recorded by promoting the escalation to charter Amendment A5; A5 is a node rather than a footnote because two subtasks are gated on it, T13 load-bearingly (A5 carries the Tier-2 grant for the exclusion-surface extension charter §3 M4 otherwise forbids). `A5 --> T11` and `A5 --> T13`: neither packet is emitted before A5 lands. `T11 --> T13` is **ordering, not artifact consumption** — T11 opens the Decision 2 debt row that T13 either closes or leaves open (ESC-M4-1 §6 item 3), and T11's F-03 registry filing must describe the pre-fix exclusion state so T13's change reads as a state transition rather than a contradiction. `T7 --> T11`: T11 writes an eval-debt row through T7's landed `open_debt` path and bumps `open_debt_high_water_mark` per runbook Step 7. `T2 --> T13`: T13 edits `bootstrap.py`'s exclusion loader, whose CLI wrapper T2 froze — T13 must not alter any §2.6 string.

**F-04 ownership, resolved (v1.6).** ESC-M4-1 §6 item 2 ruled *restore*, so the post-T13 Elder Care exclusion population returns to 5 and `GAP-103`'s existing "5 aggregate_exclude/no_citation_source" claim becomes true again. **F-04 closes in T13** by restored truth plus an evidence line — not by prose edit, and not in T11, whose packet explicitly disclaims the row. One finding, one owner, stated in both packets.

**v1.5 amendment edges (audit remediation).** `T9 --> ESCM41`: the escalation's subject matter is T9's re-run output (`0a5b2f2`) and the pilot's committed gold (`clearsulting.yaml`); ESC-M4-1 cannot be ruled without both. `ESCM41 --> T11`: T11's Files-to-touch (`registry.yaml`'s `GAP-103` row, Elder Care's exclusion annotations, Clearsulting's debt/registry filing) is gated on the operator's Decision 1–3 rulings — T11 has no packet until ESC-M4-1 resolves. `T9 --> T12`, `T10bis --> T12`, `T3 --> T12`, `T7 --> T12`, `T8 --> T12`: T12 closes coverage gaps against code all five subtasks landed (T10-bis's adapter for F-05, T8's runbook extractor for F-06, T3's `harness_cli` for F-07/F-09/F-15, T7's ledger for F-08, T6's `trust_statement.py` for F-10) — no new design decisions, so T12 is unblocked and may start immediately, in parallel with ESC-M4-1's resolution.
...
### T11 — Amendment: registry & gold-corpus honesty remediation (audit F-02 record, F-03; F-04 conditionally) — ESC-M4-1 Decisions 2 and 4

| Field | Content |
|---|---|
| **ID** | T11 |
| **Scope** | Execute ESC-M4-1's ruled Decision 2 and Decision 4: open an eval-debt row recording Elder Care's lost `no_citation_source` provenance (F-02's record component), and file the 12 bloated Clearsulting `filename_closure` intents as a registry row plus per-intent eval-debt rows with an operator disposition signoff and an explicit uninterpretability label on the pilot baseline (F-03). Records state; changes no gold and no code. |
| **Files to touch** | `eval/program/registry.yaml` (one new row for the Clearsulting bloated-gold finding; **`GAP-103-recall-at-10-bloated-gold` is not touched — F-04 is T13's**); `eval/program/eval_debt/eval_debt.yaml` (Decision 2 row + Decision 4 rows, written via `eval/retrieval/eval_debt.py`'s `open` CLI, plus the `open_debt_high_water_mark` bump per runbook Step 7); new `.dev/plans/eval-consolidation-m4-onboarding-runbook/signoffs/T11-clearsulting-bloated-gold-disposition.md`; `.dev/plans/eval-consolidation-m4-onboarding-runbook/signoffs/T9-clearsulting-pilot.md` (baseline-label addendum only — do not rewrite the walk record); `eval/program/onboarding_runbook.md` (**one caveat sentence in the per-company baseline-promotion policy section only** — see the merge strategy at §5.4 item 14, because T12 also edits this file in a different section). **Explicitly not touched:** `eval/retrieval/gold_labels/*.yaml`, `eval/retrieval/gold/gold_exclusions.yaml`, any module under `eval/retrieval/**`. Gold and code are T13's, and only under A5's grant. |
| **Contract bindings** | All of §2. §2.8 (registry writes are row edits/additions only, never schema) is load-bearing, and it binds `eval_debt.yaml` equally: represent the finding with the existing `{id, company, surface, layer, kind, opened_at, evidence_refs, closes_when}` row shape, exactly as `GAP-103`'s `kpi.retrieve_bench_and_capacity` precedent did. §2.4 applies to every `eval_debt open` invocation transcribed into the changelog. |
| **Inputs** | **Charter Amendment A5** — the promoted ESC-M4-1 ruling; must be landed before this packet is executed (kill criterion 1). **ESC-M4-1 §5** Decisions 2 and 4, quoted verbatim into the packet. **Audit F-02/F-03/F-04 evidence** — `.dev/audits/eval-consolidation/M4/2026-08-14-eval-consolidation-m4-onboarding-runbook-audit.md` §7 (the 12-intent table with positive counts, the exact `GAP-103` row text, the `registry.yaml:320-344` and `elder_care.yaml` diff citations). **T7's landed ledger** — `eval/retrieval/eval_debt.py` `open_debt` / `assert_ledger_ratchet` and the committed ledger at `open_debt_high_water_mark: 1`. |
| **Outputs** | **(a) Decision 2 — Elder Care annotation-loss debt row.** One row, `company: elder_care`, `layer: retrieval`, `kind` naming lost exclusion provenance, `evidence_refs` citing ESC-M4-1 / A5 and commit `0a5b2f2`, `closes_when` = `gold_exclusions.yaml` is company-scoped **and** Elder Care's three `no_citation_source` annotations are restored (both clauses are live — ESC-M4-1 §6 item 2 ruled *restore*, and T13 closes this row in the same round per §6 item 3). **(b) Decision 4 — F-03 registry row.** One new row (suggested `source_id`: `GAP-M4-1-clearsulting-bloated-filename-closure`) naming **each** of the 12 intents individually with its positive count and max recall@10 — not one aggregate sentence, which is the very critique F-03 makes of the pilot signoff — with `evidence_refs` pointing at `clearsulting.yaml`, the audit section, and the signoff in (d). **(c) Decision 4 — per-intent eval-debt rows** for the same 12 intents (one row per intent, or one row per intent family if the executor documents the grouping and it still names all 12), each with a `closes_when` stating what a non-bloated gold method for that intent requires; `open_debt_high_water_mark` raised to the new open count in the same diff, per runbook Step 7. **(d) Operator disposition signoff** at `signoffs/T11-clearsulting-bloated-gold-disposition.md`, mirroring `signoffs/T12-bench-disposition.md`'s shape: the 12 intents, their recall ceilings, the accepted residual risk, and the explicit statement that `baseline_7174e0399e29` **stands** and is **not** interpretable per-intent for these 12. Authored per ESC-M4-1 §6 item 4; the operator signs it. **(e) Baseline label on three surfaces**, per ESC-M4-1 §6 item 4: the registry row in (b) — the tracked surface, so the label survives `.dev/` — plus an addendum paragraph in `signoffs/T9-clearsulting-pilot.md` where a reader of the pilot record meets it, plus one caveat sentence in `eval/program/onboarding_runbook.md`'s per-company baseline-promotion policy section stating that a promoted baseline whose gold carries bloated `filename_closure` intents is not per-intent interpretable for those intents and must be labeled at promotion time. All three cross-reference (b) and (d). **(f) F-04 is not T11's** — restoration was ruled in, so `GAP-103` closes in T13 by restored truth. Do not touch that row. **(g)** Changelog. **(h)** Decision log at `decision-logs/T11.md` quoting the A5 ruling it executes. |
| **Kill criteria** | Halt and report if: charter Amendment A5 is not present at charter v0.1.4 at execution start, or ESC-M4-1 §5/§6 shows any decision as *pending* (do not guess a disposition to unblock); **or** the runbook's baseline-promotion policy section cannot carry (e)'s caveat sentence without restructuring the section — a restructure is T8's surface and a separate finding, not an executor's edit; **or** filing either finding would require a new field on `registry.yaml` or `eval_debt.yaml` (report and route per §2.8 — a schema change is a fresh escalation, not an executor's call); **or** `assert_ledger_ratchet` fails after the new rows land for any reason other than the high-water mark, which this subtask is authorized to raise (a ratchet failure on evidence-ref resolution means a cited artifact does not exist and the row is not yet true); **or** any file outside Files to touch would need editing to make the suite green. |
| **Log tier** | **architectural** — files a new class of finding against a hub artifact (gold-corpus honesty), lands an operator disposition that keeps a promoted baseline standing while declaring part of it uninterpretable, and (on the (f) branch) touches a `closed` registry row's evidentiary basis. Decision log required at `.dev/plans/eval-consolidation-m4-onboarding-runbook/decision-logs/T11.md`. |
| **Risks & mitigations** | *Risk:* the 12-intent filing understates severity if written generically instead of per-intent — exactly F-03's critique of the pilot signoff, reproduced in the remedy. *Mitigation:* Outputs (b) and (c) require each intent named with its count and ceiling; a single aggregate row is a failed T11. *Risk:* "the baseline stands but is labeled" degenerates into a label nobody meets, because it lives only in a `.dev/` signoff. *Mitigation:* the label lands in a **tracked** artifact (the registry row) as well as the signoff addendum, and ESC-M4-1 §6 item 4 is where any further surface is named — the executor does not choose surfaces silently. *Risk:* the Decision 2 debt row's `closes_when` is written against a restoration that the operator has not authorized. *Mitigation:* kill criterion 2 blocks on ESC-M4-1 §6 item 2 rather than letting the executor pick a clause. |

### T12 — Amendment: coverage-gap remediation (audit F-05, F-06, F-07, F-08; minors F-09, F-10, F-13)

| Field | Content |
|---|---|
| **ID** | T12 |
| **Scope** | Close five coverage-gap/contract-drift majors and three minors, all against code already landed by T3, T6, T7, T8, T10-bis — no new design decisions, no operator input required. |
| **Files to touch** | `eval/retrieval/gold/bootstrap.py` and `eval/retrieval/tests/test_gold_kpi_pdf_branch.py` (F-05); `eval/retrieval/tests/test_onboarding_runbook.py` and `.dev/onboarding_cluster_submit.py` (F-06); `eval/retrieval/harness_cli.py` (F-07, F-09); `eval/retrieval/eval_debt.py` and `eval/retrieval/tests/test_eval_debt.py` (F-08); `eval/program/onboarding_runbook.md` §2.6-quoting line 2 and this plan's own §2.6 (F-09); `eval/retrieval/trust_statement.py` (F-10); `eval/program/onboarding_runbook.md` (F-13, informational note only — see below). |
| **Contract bindings** | All of §2. §2.4's one required field discipline (company slug + catalog on every CLI summary line) is F-07's binding target. §2.6's frozen strings are F-09's binding target — correcting §2.6 itself is part of this subtask's Outputs, per the Freeze order's own change-report clause. §2.1's typed-surface table gets no new rows (all five fixes are behavior/test additions to already-typed surfaces), except F-10, which **adds** `display_name` derivation to §2.1's typed-surface table naming `canonical_company_slug`'s existing fold as the required mechanism (see Outputs). |
| **Inputs** | **T10-bis** (landed adapter, for F-05); **T8** (landed runbook + extractor, for F-06); **T3** (landed `harness_cli`, for F-07/F-09/F-15 residual); **T7** (landed ledger, for F-08); **T6** (landed `trust_statement.py`, for F-10). All landed at HEAD; to be supplied at execution time by reading the current tree, not re-derived from packets. |
| **Outputs** | **(F-05)** Two new tests in `test_gold_kpi_pdf_branch.py`: (i) a synthetic over-broad-pattern case (a common section title, e.g. `Overview`, that matches a second unrelated `section_header` in the fixture chunk set) asserting the resolved positive set is the exact expected chunk IDs, not merely non-empty, catching the case where the pattern over-matches; (ii) a no-page-location case (`Section: X` with no `, Page N` suffix) asserting the query's narrowing behavior is what the citation supports (i.e., document-scoped, not falsely page-scoped) — both per T10-bis's own kill criterion (3) and Risks & mitigations, which named this exact gap and were shipped without it. **(F-06)** Register `.dev/onboarding_cluster_submit.py`'s `build_parser()` in `test_onboarding_runbook.py`'s `_MODULE_PARSERS`, and widen `_extract_bash_commands` (or add a second extractor) so runbook lines invoking `python .dev/onboarding_cluster_submit.py …` are parsed against that parser, not merely string-matched — replacing the presence-only assertions in `test_runbook_documents_serverless_cluster_path` with an actual parse check for both the `bootstrap` and `harness-baseline` cluster invocations at runbook lines 75 and 137. **(F-07)** Add a §2.4-conforming summary line to `harness_cli run` (currently bare `report.manifest.run_id`) and `validate-baseline` (currently bare `"baseline_ref validation passed"`), each naming the resolved company slug and catalog; optionally extend `exemptions list` and `eval_debt list` similarly (audit marks these `minor`, not blocking — do at the executor's discretion within the same diff). **(F-08)** Add a test asserting that after `close_debt`, the ledger's set of debt `id`s is a superset of the pre-close set (i.e., closing does not shrink the id set) — proving deletion-as-closure is impossible by construction, not merely undocumented; if the executor judges a test infeasible, the alternative is a registry follow-up row with an explicit ID, added to §8.4.3's existing waiver line in place of "no follow-up ID". **(F-09)** Correct plan §2.6 line 2 to include `--run-type <baseline|comparison>` (or whatever the landed parser's actual required-argument set is — read `harness_cli.py`'s `build_parser()` directly, do not guess), and correct Validation record item 8 from "PASS by construction" to state what was actually verified. **(F-10)** Either (i) add `merge_exemption_companies_into_domain`'s `display_name` derivation to §2.1's typed-surface table with `canonical_company_slug` as the named forward-fold and a round-trip test proving `slug.replace("_"," ").title()` is stable for every company the domain currently carries, or (ii) replace the inverse fold with a stored/looked-up display name if one already exists on the exemption or company record — executor's call, document which in the changelog. **(F-13)** No code change required (audit marks this `minor`/mitigated — the file is force-added and resolves at HEAD); Outputs is a single sentence added to the runbook's serverless section noting the `.dev/` Option C caveat explicitly, so a future mover of the file is warned rather than surprised. Changelog. Decision log only if the executor's F-10 or F-05 choice involves a real design fork (see Log tier). |
| **Kill criteria** | Halt and report if: the F-05 over-broad-pattern test cannot be constructed without live warehouse access (T10-bis's own tests already prove `MockSpark` suffices for this module — if T12's executor finds otherwise, that is itself a new finding to report, not a reason to skip the test); **or** `harness_cli.py`'s actual required-argument set for `run`/`validate-baseline` differs from what F-09's Outputs assumes — read the parser, don't guess, and correct §2.6 to match what is actually there; **or** any of the five fixes would require touching `trust_statement.py`'s C6 hub surface beyond the F-10 derivation itself (that would be a new coupling, out of this subtask's scope). |
| **Log tier** | **standard** — five independent, mechanical coverage-gap closures against already-landed, already-typed surfaces; no new architectural fork. (If the executor's F-10 resolution requires replacing the inverse-fold mechanism with a new stored field, that sub-decision alone is promoted to architectural with its own decision log entry — flagged in advance so the executor knows the threshold.) |
| **Risks & mitigations (T12)** | *Risk:* F-05's synthetic fixture might not faithfully reproduce the real corpus's over-match risk (a lab-only test that doesn't generalize). *Mitigation:* base the fixture's ambiguous section title on the real corpus fact already in evidence — T10's own decision log records the unresolvable content living under `Overview` and `Description of adjustment`, so a fixture using a repeated `Overview` header is not invented, it mirrors a confirmed corpus shape. *Risk:* F-06's extractor widening could over-broadly start parsing every `bash` block in the runbook, including illustrative or non-command prose. *Mitigation:* scope the second extractor to lines matching the `.dev/onboarding_cluster_submit.py` prefix specifically, mirroring the existing prefix-filter pattern rather than removing the filter. |

### T13 — Amendment: company-scoped gold exclusions (audit F-02 structural; §5.4 coupling 12) — ESC-M4-1 Decision 3(b)

| Field | Content |
|---|---|
| **ID** | T13 |
| **Scope** | Remove the cross-company write path at its root: `gold_exclusions.yaml` becomes keyed by company, `load_gold_exclusions` requires a company slug and returns only that company's exclusions, and the global claim-map/exclusion disjointness invariant is re-expressed per company so that onboarding one company can no longer de-annotate another company's committed gold. T13 then restores Elder Care's three `no_citation_source` annotations (A5-granted), closes the F-04 registry claim by restored truth, and closes T11's Decision 2 debt row. |
| **Files to touch** | `eval/retrieval/gold/gold_exclusions.yaml` (reshape to the §2.1 company-keyed form; Elder Care's launch population **plus** the three intents `0a5b2f2` removed; Clearsulting's own block, which is empty today); `eval/retrieval/gold/bootstrap.py` (`load_gold_exclusions`, `GoldLabelBootstrap._gold_exclusions` and its cache key, and the `_bootstrap_pass1` call site at ~line 418); `eval/retrieval/tests/test_gold_exclusions.py` (the reshaped invariant plus the three new §2.1 tests); `eval/retrieval/tests/test_gold_bootstrap.py` (the `load_gold_exclusions` call at line 322 and the `EXPECTED_AGGREGATE_EXCLUDE_COUNT` / population constants); `eval/retrieval/gold_labels/elder_care.yaml` (the three rows for `kpi.retrieve_bill_rates_and_margins`, `kpi.retrieve_headcount_attrition`, `kpi.retrieve_pipeline_backlog` — **A5's grant covers this write and no other row, no other company**); `eval/program/registry.yaml` (`GAP-103-recall-at-10-bloated-gold` evidence line recording that the population is 5 again and why — this is F-04's closure); `eval/program/eval_debt/eval_debt.yaml` (close T11's Decision 2 row via `close_debt`; **do not lower `open_debt_high_water_mark`**). **Explicitly not touched:** `eval/retrieval/gold/kpi_claim_intent_map.yaml`, `eval/retrieval/gold_labels/clearsulting.yaml`, `eval/retrieval/harness.py`, `eval/retrieval/trust_statement.py`. |
| **Contract bindings** | All of §2. The two new §2.1 rows are this subtask's primary output. §2.2's write-path fold rule binds: the company key in the artifact and in every lookup is a folded slug produced by `companies.canonical_company_slug`, never a display name and never a locally re-implemented fold. §2.2's error envelope is otherwise unchanged — `PreconditionError` stays the only raise this loader contributes, now additionally for a missing/unfolded `company_slug` argument. §2.5 in full: hermetic, `MockSpark`, tests in the same diff. §2.6 is untouched — no CLI string changes. §1's rows-not-schema non-goal binds: the company dimension is added to the config artifact, **not** to `GoldLabel`. |
| **Inputs** | **Charter Amendment A5** — carries the Tier-2 grant for this surface extension; without it this subtask violates charter §3 M4's "no new eval-surface design" non-goal (kill criterion 1). **ESC-M4-1 §5 Decision 3(b) and §6 items 2 and 3**, quoted verbatim into the packet. **T11's landed debt row** — its `id` and `closes_when`, needed for the closure. **Audit F-02** — `.dev/audits/eval-consolidation/M4/2026-08-14-eval-consolidation-m4-onboarding-runbook-audit.md` §7 F-02 (the mechanism, the metric-impact verification at `harness.py:253-262,778-782`, and the disjointness assertion at `test_gold_exclusions.py:143-144`). |
| **Outputs** | **(a)** The company-keyed artifact and the two §2.1 typed surfaces, with the three named tests. **(b)** The reshaped invariant. Today `test_gold_exclusions_totality_and_disjointness` asserts global disjointness between claim-mapped intents and excluded intents, which is the assertion that *forces* the cross-company rewrite. It must be re-expressed as a per-company statement — for each company block, that company's excluded intents are disjoint from the intents that company's own gold shows claim-resolved — with the totality half (`excluded_kpi == unmappable_kpi` for the company whose corpus defined the launch population) preserved for Elder Care and stated as company-scoped rather than global. The executor chooses between deriving "claim-resolved for this company" from the committed gold file or from a company-scoped view of the claim map, and **must document the choice and its rejected alternative in the decision log**. **(c) Restoration (ESC-M4-1 §6 item 2, ruled *restore*):** the three Elder Care rows regain `aggregate_exclude: true`, `exclude_reason: no_citation_source` and the `aggregate_exclude: …` note form, byte-compatible with their pre-`0a5b2f2` shape; a regression test asserts Elder Care's exclusion population is 5 and Clearsulting's mapping of the same three intents does not change it. `GAP-103`'s row gains an evidence line stating the population is 5 again, which closes **F-04** by restored truth rather than prose edit. **(d) Closure (§6 item 3, ruled *close in-round*):** T11's Decision 2 debt row is closed with `close_debt`, `closed_evidence_refs` citing this subtask's commit and A5, leaving the row present per T7's design; `open_debt_high_water_mark` stays where T11 set it, because a mark that follows the open count down is not a ratchet (§5.4 item 15). **(e)** Verification that Elder Care's committed metrics are unchanged: the harness skip predicate treats `aggregate_exclude` and `bootstrap_failed` as alternatives (`harness.py:253-262,778-782`), so restoring the annotation must move **no** metric — assert it, do not assume it. **(f)** Changelog. **(g)** Decision log at `decision-logs/T13.md`: the A5 grant quoted, the invariant-reshape fork with its rejected alternative, and an explicit statement that this closes §5.4 coupling 12 and the audit's scout-miss heuristic. |
| **Kill criteria** | Halt and report if: **(1)** charter Amendment A5 is absent from the charter at v0.1.4, or does not name both the exclusion artifact and `eval/retrieval/gold_labels/elder_care.yaml` as granted M4 surfaces — absent that grant this work is a charter violation, not an amendment; **(2)** the per-company invariant in (b) cannot be closed without **also** company-scoping `eval/retrieval/gold/kpi_claim_intent_map.yaml` — that is a second hub artifact and a second Tier-2 grant, so stop, report, and let the charter decide whether the remainder moves to M5 (this is the split trigger named in the v1.6 carryability note); **(3)** reshaping the artifact would require a `GoldLabel` field, a new `exclude_reason` vocabulary member, or a `registry.yaml` / `eval_debt.yaml` schema change (all forbidden by §1 and §2.8); **(4)** restoring the three Elder Care rows changes any committed Elder Care metric, baseline number, or trust-statement row — the audit verified it must not, so a diff there means the skip predicate is not what F-02 established and the restoration stops until that is understood; **(5)** any Clearsulting gold row changes as a side effect of the reshape (the pilot's committed gold is not this subtask's to move); **(6)** making the suite green requires editing an assertion whose failure is evidence rather than staleness — a test that fails because a company's exclusions genuinely changed is a finding, not a constant to update, which is precisely the pattern `0a5b2f2` was faulted for at audit CR-2. |
| **Log tier** | **architectural** — changes the shape of a committed config hub artifact, changes a public loader signature, re-expresses a program invariant, and writes a second company's gold corpus under an explicit Tier-2 grant. Decision log required at `.dev/plans/eval-consolidation-m4-onboarding-runbook/decision-logs/T13.md`. |
| **Risks & mitigations** | *Risk:* this is scope creep under audit pressure, by the operator's own characterization of Decision 3(b) — the fix could grow into a general per-company gold-config redesign. *Mitigation:* kill criterion 2 is the hard stop, and Files to touch names `kpi_claim_intent_map.yaml` and `clearsulting.yaml` as explicitly excluded. *Risk:* the reshape lands but the invariant is quietly weakened rather than re-scoped — a per-company assertion that asserts nothing is worse than the global one it replaced, because the cross-company defect would then be undetectable instead of merely painful. *Mitigation:* Outputs (b) requires the new invariant to fail on a synthetic fixture where one company's claim mapping contradicts another's exclusion; the decision log must show that failing case. *Risk:* restoration is treated as a revert of `0a5b2f2`. *Mitigation:* it is not — the claim-map additions and Clearsulting's live claim targets stay; only Elder Care's annotations return, now legally, because exclusions are company-scoped. The decision log must say this in those terms. |
...
14. **(v1.6)** `(The claim→intent map can stay company-agnostic while exclusions become company-scoped — i.e. "this company has no citation source for intent I" is expressible per company without also partitioning kpi_claim_intent_map.yaml | §2.1's two v1.6 rows; eval/retrieval/gold/kpi_claim_intent_map.yaml; test_gold_exclusions.py::test_gold_exclusions_totality_and_disjointness | if the per-company invariant cannot be stated without a company-scoped claim map, T13's reshape either weakens the invariant to vacuity or silently expands to a second hub artifact — the first is a worse defect than F-02 itself, the second is an ungranted charter extension; T13 kill criterion (2) is the falsifier and the split trigger | T13)`
15. **(v1.6)** `(Restoring Elder Care's three aggregate_exclude annotations moves no metric, because harness.py treats aggregate_exclude and bootstrap_failed as alternatives in one skip predicate | harness.py:253-262, harness.py:778-782 | if the predicate is not the alternation the audit read it to be, restoration silently changes M1/M3 landed Elder Care evidence — a closed-gate regression reached through an audit remediation, which is the worst available outcome for this round | T13)` — the audit verified this directly and the claim is therefore **confirmed-by-audit**; T13 kill criterion (4) re-asserts it at execution time rather than trusting the reading.
16. **(v1.6)** `(A retroactive ratification closes F-01 without setting a precedent, because A5 says so | ESC-M4-1 §5 Decision 1; charter Amendment A5 (pending) | if A5 records the ratification without the non-precedential clause, the program's standing answer to "may I land gold outside the routing table?" becomes yes-if-the-code-is-right, and §7's routing table stops binding | A5-authoring, T11)`
...
12. **(v1.5) confirmed — audit scout-miss, now closed by disclosure** — `(gold_exclusions.yaml is keyed by intent_id only (company-agnostic), while every consumer that writes against it — a company's claim-mapping work, the global disjointness invariant in test_gold_exclusions.py — is keyed by (intent_id, company) | eval/retrieval/gold/gold_exclusions.yaml + eval/retrieval/tests/test_gold_exclusions.py::test_gold_exclusions_totality_and_disjointness | onboarding any company that gains a claim mapping for an intent another company has excluded silently rewrites that other company's committed gold annotation, with no coupling tuple catching it before it happens | T9,T11)` — evidence: audit finding F-02, confirmed live in `0a5b2f2` against Elder Care. This tuple was **not** catalogued at planning time (the context map classified the file `transitive` with no company-dimension note); added here in v1.5 as the disclosed record, per the audit's own recommended heuristic: "when a file is keyed by one dimension but consumed in a context keyed by two, catalogue it as a coupling surface regardless of traversal depth." **Open** pending ESC-M4-1 Decision 3 — carried to T11 rather than closed here, since the fix (if any, this round) is T11's Files-to-touch, not a §5.4 narrative edit. **v1.6 update:** Decision 3 ruled **(b)**, fix now — this coupling's owner is **T13**, not T11, and it closes when T13's per-company invariant lands with the failing-fixture demonstration required by T13 Outputs (b). Until then it stays **open**.
13. **(v1.6) confirmed** — `(The exclusion-population test constants are a shared surface between T13's reshape and the constants 0a5b2f2 already moved once | eval/retrieval/tests/test_gold_exclusions.py (LAUNCH_EXCLUDED_KPI, the population-count test) + eval/retrieval/tests/test_gold_bootstrap.py (EXPECTED_AGGREGATE_EXCLUDE_COUNT, load_gold_exclusions call at line 322) | these constants were edited to match new data once already, which is the pattern audit CR-2 flagged; if T13 edits them again to make the suite green, the diff is indistinguishable from the defect being remediated unless T13's changelog states which constant moved, for which company, and why the movement is the contract rather than an accommodation of it | T13)` — evidence: `git show 0a5b2f2 -- eval/retrieval/tests/test_gold_exclusions.py eval/retrieval/tests/test_gold_bootstrap.py`; T13 kill criterion (6) is the guard.
14. **(v1.6) confirmed — parallel-safety, with a documented merge strategy** — `(T11 and T12 both edit eval/program/onboarding_runbook.md while running in parallel | eval/program/onboarding_runbook.md + eval/retrieval/tests/test_onboarding_runbook.py | two agents editing one markdown file in the same round produce a merge conflict at best and a silently reverted caveat at worst — and the file is the milestone's contract surface, the object G7 tests | T11,T12)` — **merge strategy, which is why these stay parallel rather than sequenced:** the edits are section-disjoint and additive. **T12** touches only the *serverless / cluster-execution* section (F-13's `.dev/` Option C caveat sentence) and the test module. **T11** touches only the *per-company baseline-promotion policy* section (F-03's uninterpretability caveat). Neither may reflow, retitle, or reorder any section it does not own; neither may touch a `bash` block, since those are T12's parse-guard surface and any command-text change breaks the other's test. Whichever lands second rebases its single sentence onto the other's tree without re-editing it. If either finds it must edit the other's section, that is a report, not a resolution.
15. **(v1.6) suspected** — `(T11 and T13 both write eval/program/eval_debt/eval_debt.yaml and both touch open_debt_high_water_mark | eval/program/eval_debt/eval_debt.yaml; eval/retrieval/eval_debt.py open_debt / close_debt / assert_ledger_ratchet | T11 raises the mark for its new rows and T13 closes one of them in the same round; if the two diffs interleave or T13 lowers the mark to match the new open count, the ratchet's meaning inverts — a high-water mark that follows the open count downward stops being a ratchet | T11,T13)` — disprovable at T13 execution time by asserting the mark is monotonic non-decreasing across the two commits; the `T11 --> T13` ordering edge exists partly for this reason.
...
| `packets/T11.md` | **new (v1.6)** | emitted after ESC-M4-1 §5/§6 ruled and A5 landed; no conditional branches — F-04 is explicitly disclaimed to T13, `closes_when` and the three label surfaces are fixed |
| `packets/T12.md` | **new (v1.5)** | audit-remediation packet; self-contained per §6; unblocked, no operator decision required — **executable now** |
| `packets/T13.md` | **new (v1.6)** | emitted after A5 landed (the Tier-2 grant is T13's kill criterion 1); restoration and closure are in scope per ESC-M4-1 §6 items 2–3 and are stated unconditionally |

**Retired-string sweep (v1.6).** The v1.6 §2.1 additions change `load_gold_exclusions`'s signature (a `company_slug` keyword becomes required) and the `gold_exclusions.yaml` top-level key (`excluded:` → `companies: {<slug>: {excluded: […]}}`). Both are **retired strings from T13's landing onward.** Swept every emitted packet (`rg "load_gold_exclusions|gold_exclusions" packets/`): the only hit is `packets/T9.md:13`, a historical statement that `0a5b2f2` "narrowed `gold_exclusions.yaml` to profiler + healthcare_labor_market only." That sentence is a **completed-pilot record, not a forward instruction**, and T9 is landed — the packet is left unmodified for the same reason `packets/T10.md` is (audit trail), and T13's changelog must note that the sentence describes the pre-T13 shape. No other packet references the loader or the artifact, so no re-emission is needed. The live consumers are all inside T13's own Files to touch (`bootstrap.py`, `test_gold_exclusions.py`, `test_gold_bootstrap.py`), which is why the reshape is one subtask and not a cascade. `packets/T11.md` and `packets/T13.md` are authored after this sweep and must quote the post-T13 forms only. No §2.6 CLI string is affected.
...
### ESC-M4-1 — RB-DEFECT-T9-1 gold-corpus write (Tier 2, retroactive; audit F-01, F-02)

**Trigger.** The 2026-08-14 audit (`.dev/audits/eval-consolidation/M4/2026-08-14-eval-consolidation-m4-onboarding-runbook-audit.md`) found that commit `0a5b2f2` (RB-DEFECT-T9-1) rewrote three of Elder Care's committed gold rows and the company-agnostic `gold_exclusions.yaml` with no packet, decision log, changelog, or charter routing — F-01 (process-violation) and F-02 (undeclared-change), both major.

**Blast-radius classification.** **Not** within M4's §7 amendment boundaries. Per this plan's own pre-committed routing table (below), a change reaching a gold corpus — here, a second company's committed gold, via a company-agnostic exclusion file — is explicitly excluded from the §7 path: *"absorbing hub scope via amendment bypasses the charter's serialization constraint."* Routed to Tier 2.

**Escalation artifact.** `.dev/plans/eval-consolidation-m4-onboarding-runbook/escalations/ESC-M4-1-rb-defect-t9-1-gold-corpus.md` — drafted 2026-08-14, **RULED 2026-08-14** on four decisions (retroactive governance record; Elder Care annotation-loss disposition; structural-fix timing; F-03 disposition, added at ruling time). Not yet promoted to a numbered charter amendment; promotion to **Amendment A5** is the next action, following the A1–A4 precedent in `eval_consolidation_program_milestone_charter.md`.

**Ruling, as it binds this plan (v1.6).**

| Decision | Ruling | Closes | Executed by |
|---|---|---|---|
| 1 | Ratify `0a5b2f2` retroactively; the record is A5, which must state the ratification is **retroactive and non-precedential** | F-01 | A5 promotion (no code) |
| 2 | Eval-debt row for Elder Care's lost `no_citation_source` provenance | F-02 (record) | T11 |
| 3 | Company-scope `gold_exclusions.yaml` **now** | F-02 (structural), §5.4 coupling 12 | **T13** |
| 4 | F-03 as registry row + per-intent debt rows + operator disposition signoff; baseline stands, labeled uninterpretable per-intent; no fresh Tier-3 | F-03 | T11 |

**What A5 grants, not merely records — landed.** Decision 3(b) extends a hub contract surface — the gold-exclusion config, plus a second company's gold corpus for the restoration — beyond what charter §3 M4's block grants, and charter §3 M4's non-goals explicitly exclude "any new eval-surface design beyond what the runbook documents." A5 therefore (a) names `eval/retrieval/gold/gold_exclusions.yaml` and `eval/retrieval/gold_labels/elder_care.yaml` (restoration write only) as M4-granted surfaces, in the amendment text **and** in the §3 M4 "Contract surfaces extended" cell, and (b) carries Decision 1's non-precedential clause. The grant is bounded: `kpi_claim_intent_map.yaml`, `clearsulting.yaml`, the `GoldLabel` schema and the `exclude_reason` vocabulary stay out. Without (a), T13 would be a charter violation rather than a granted amendment, which is why verifying it is T13's first kill criterion.

**Downstream gate.** T11 and T13 have no packets and do not start until A5 lands and ESC-M4-1 §6's residual items are answered. T12 is independent and unblocked.

### T11 — F-02 record + F-03 disposition (ESC-M4-1 Decisions 2 and 4)

**Trigger.** Audit findings F-02 (record component) and F-03, ruled by ESC-M4-1 Decisions 2 and 4.

**Blast-radius classification.** Within M4's boundaries: T11 writes only program-state records — `registry.yaml` rows, `eval_debt.yaml` rows, `.dev/` signoffs. It touches no gold corpus, no module, and no CLI surface. §7 amendment path applies.

**Explicit DAG edges from every implicated consumer.** `A5 --> T11` (T11 executes A5's ruling verbatim and quotes it in its decision log). `T7 --> T11` (T11 writes through T7's landed `open_debt` and is subject to `assert_ledger_ratchet`; it raises `open_debt_high_water_mark`, which is T7's committed mechanism). `T11 --> T13` (ordering: the Decision 2 row must exist before T13 can close it, and T11's filing must describe the pre-fix state).

**Scope discipline.** Records only. No gold edit, no code edit, no schema field. The one conditional branch — F-04's prose correction — exists **only** if restoration is declined, and the packet states which branch is live rather than leaving the executor to infer it.

**DoD — code and narrative.** (a) Records: the rows, the signoff, the label addendum, the high-water-mark bump. (b) Narrative: this §7 subsection plus §5.4 item 12's v1.6 update are the back-annotation; T11's decision log quotes the A5 ruling and records that F-04 is T13's, not its own. (c) §8.4.1/§8.4.2 gain dispositions for §5.2 items 14–16 and §5.4 items 12–15 at the next *Complete* banner.

### T12 — Coverage-gap remediation (audit F-05 – F-08; minors F-09, F-10, F-13)

**Trigger.** Audit findings F-05, F-06, F-07, F-08 (majors) and F-09, F-10, F-13 (minors, recommended in the same pass).

**Blast-radius classification.** Within M4's boundaries. Tests and summary lines against already-landed, already-typed surfaces; one plan-narrative correction (§2.6 line 2 and Validation record item 8). No hub extension, no operator decision, no new design fork.

**Explicit DAG edges from every implicated consumer.** `T3 --> T12`, `T7 --> T12`, `T8 --> T12`, `T9 --> T12`, `T10bis --> T12` — one edge per subtask whose landed code T12 adds a falsifier or a summary line to.

**Scope discipline.** No behaviour change beyond the two `harness_cli` summary lines and F-10's display-name derivation; every other output is a test or a narrative correction. F-13 is a single documentation sentence, not a path move.

**DoD — code and narrative.** (a) Code: the falsifiers and summary lines land with the diff. (b) Narrative: §2.6 line 2 is corrected in place and Validation record item 8's false PASS is restated as what was actually verified — that correction is part of T12's Outputs, not a later cleanup. (c) F-08's fallback path, if the persistence test proves infeasible, is a named registry follow-up ID attached to §8.4.3's waiver line, replacing "no follow-up ID" — the audit's own downgrade condition.

### T13 — Company-scoped gold exclusions (ESC-M4-1 Decision 3(b))

**Trigger.** Audit F-02's structural component and §5.4 coupling 12: `gold_exclusions.yaml` is keyed by `intent_id` alone while every consumer is keyed by `(intent_id, company)`, and the global disjointness invariant makes the cross-company rewrite mandatory rather than accidental. Ruled by ESC-M4-1 Decision 3(b), fix now.

**Blast-radius classification.** **Hub extension — granted by Tier-2 ruling, not absorbed by amendment.** This is the case §7 reserves for the charter: a gold-config surface reshape, plus a write to a second company's committed gold. It is carried inside this plan **only** because Decision 3(b) is the Tier-2 grant and A5 records it; the §7 machinery below governs execution, not authorization. If T13's kill criterion (2) fires, authorization has run out and the remainder routes to the charter for an M5 slice.

**Explicit DAG edges from every implicated consumer.** `A5 --> T13` (load-bearing — the grant). `T11 --> T13` (ordering + the debt row T13 may close). `T2 --> T13` (T13 edits the module T2 froze a CLI over; no §2.6 string may move). Consumers checked and deliberately **not** edged: `harness.py` and `trust_statement.py` read `aggregate_exclude` off committed gold rows, never the exclusion config, so the reshape cannot reach them — asserted by T13 kill criterion (4) rather than assumed.

**Scope discipline.** The company dimension is added to one config artifact and one loader signature. Not in scope: `kpi_claim_intent_map.yaml`, Clearsulting's gold, `GoldLabel`'s fields, the `exclude_reason` vocabulary, any CLI string, any registry/ledger schema field. Two of those are kill criteria rather than mere exclusions because they are the plausible growth directions.

**DoD — code and narrative.** (a) Code: the reshaped artifact, the company-scoped loader, the per-company invariant with its failing-fixture demonstration, the Elder Care restoration and the debt-row closure — all with tests in the same diff (§2.5). (b) Narrative: §2.1's two v1.6 rows are the forward contract; §5.4 item 12 moves from **open** to **closed** with T13's commit as evidence; §5.2 items 14–15 are dispositioned; and T13's decision log records the invariant-reshape fork with its rejected alternative. (c) `GAP-103`'s evidence line is T13's and F-04 closes there; T11's packet disclaims the row explicitly, so exactly one subtask owns the finding.
...
| 8 | CLI strings frozen before downstream packets | PASS — T12 corrected §2.6 line 2 to include required `--run-type baseline`; verified by reading `harness_cli.build_parser()` and `test_onboarding_runbook.py` parse checks (runbook was already correct at line 147) |
...
| Field | Value |
|---|---|
| Gate SHA (HEAD) | `1a2daeb82a60f070c2c1eb3eaf661509b38388d4` — commit `m4 completed -> auditor handoff` |
| Working tree @ gate SHA | **clean** — handoff commit includes plan §8 (v1.4), T9 decision-log/signoff touch-ups, `CHANGELOG.MD`, and test assertion fixes in `test_spot_check.py` / `test_trust_statement.py` |
| Verification command | `pytest -q` from repo root |
| Result @ gate SHA (2026-08-14, **clean detached worktree**) | **1153 passed, 23 skipped, 0 failed** in 21.07s · exit **0** · Python 3.13 / pytest 9.x |
| Supplementary (operator tree, same SHA) | With Option C `.dev/eval-program/company_slug_vectors.yaml` present on disk, `test_companies.py` and `test_g1_scorer_semantics_guard.py` collect 14 additional golden-vector tests → **1187 passed, 1 skipped, 0 failed** in 19.73s · exit **0** — not the binding falsifier; documents local Option C artifact presence |
| Pytest evidence | `.dev/audits/eval-consolidation/M4/m4-pytest-evidence.md` (records pre-handoff dirty-tree run @ `e3ce0bd`; auditor should treat §8.1 table above as authoritative) |
...
Auditor re-runs pytest on a **clean bare worktree** @ gate SHA as Phase 2 step 1 (M3 R8/R14 discipline). Expected binding counts: **1153 / 23 / 0** unless Option C `.dev/` artifacts are also present on the audit machine.
...
|---|---|---|---|
| 0 | This plan (v1.4 §8 body) | `.dev/plans/eval-consolidation-m4-onboarding-runbook/plan.md` | Frozen @ HEAD (`handoff -> audit`); `git hash-object` @ commit time (Option C) |
| 1 | Context map | `.dev/plans/eval-consolidation-m4-onboarding-runbook/context-map.md` | Promoted from `_pending/` at planning; **staleness:** map SHA `3126c2b` ≠ gate SHA `1a2daeb` — treat file map as planning baseline, not post-execution inventory |
...
| H1 | M4 pytest evidence | `.dev/audits/eval-consolidation/M4/m4-pytest-evidence.md` | Pre-handoff record @ `e3ce0bd` dirty tree; **§8.1 table above is authoritative** @ `1a2daeb` |
| H2 | M4 upstream handoff | `.dev/audits/eval-consolidation/M4/m4-upstream-handoff.md` | Auditor cold-read routing; SHA ladder ends @ `e3ce0bd` — extend to `1a2daeb` per §8.1 |
| F1 | Fan-in CHANGELOG | `9c4e619` | T1–T4, T2, T3, T9, T10-bis entries |
| F2 | Handoff commit | `1a2daeb` | Plan §8 (v1.4), T9 doc touch-ups, test assertion fixes, `CHANGELOG.MD` |
...
|---|---|---|
| `bootstrap.main` / `--company` / `--catalog` / `--output` | `eval/retrieval/gold/bootstrap.py:build_parser`, `bootstrap.main` | `test_gold_bootstrap.py::test_main_parses_company_catalog_output`, `test_main_output_defaults_to_company_gold_path` |
| `harness.default_gold_path(company_slug)` | `eval/retrieval/harness.py:default_gold_path` (required slug param) | `test_harness_fixture.py::test_default_gold_path_requires_slug` |
| `EvalHarness.__init__(company_slug=…)` | `eval/retrieval/harness.py:EvalHarness.__init__` | `test_harness_fixture.py::test_harness_resolves_gold_path_from_company_slug` |
| `harness_cli run --gold-path` derivation | `eval/retrieval/harness_cli.py` run subcommand | `test_harness_cli.py::test_run_derives_gold_path_from_company_name` |
| `IntentExemption` + store roundtrip | `eval/retrieval/exemptions.py:IntentExemption`, `eval/program/eval_exemptions.yaml` | `test_exemptions.py::test_roundtrip_write_then_load`, `test_committed_store_validates` |
| `run_ingest_preflight` both backends | `eval/retrieval/ingest_preflight.py:run_ingest_preflight` | `test_ingest_preflight.py::test_both_backends_satisfy_return_contract` |
| §8.4 probe boundary (no raise) | `eval/retrieval/ingest_preflight.py` + `trust_statement.py:IngestProbeResult` re-export | `test_ingest_preflight.py::test_unknown_backend_rejected`; `test_trust_statement.py` C6 wiring |
| Eval-debt ledger | `eval/retrieval/eval_debt.py:EvalDebtRow`, `eval/program/eval_debt/eval_debt.yaml` | `test_eval_debt.py::test_ledger_roundtrip` |
| Onboarding runbook CLI surface | `eval/program/onboarding_runbook.md` | `test_onboarding_runbook.py::test_runbook_commands_match_cli_surface` |
| T10-bis KPI PDF branch (§2.1 landed row) | `eval/retrieval/gold/bootstrap.py:_normalize_kpi_pdf_location`, `_positives_from_kpi_citations` | `test_gold_kpi_pdf_branch.py::test_kpi_pdf_branch_resolves_section_location`, `test_kpi_pdf_branch_zero_chunks_skips_without_raising`, `test_kpi_pdf_branch_intent_degrades_to_bootstrap_failed_when_no_citation_resolves` |
| Error envelope (new types) | `eval/retrieval/exemptions.py:ExemptionValidationError`; `ingest_preflight.py:IngestPreflightError`; `eval_debt.py:EvalDebtError` | raised-path tests in respective `test_*.py` modules |
| Naming / paths (§2.3) | frozen paths under `eval/program/` and `eval/retrieval/gold_labels/` | `test_onboarding_runbook.py`, `test_exemptions.py::test_committed_store_validates`, harness fixture tests |
...
| 10 | `pytest.ini` testpaths reach every new test | **closed** | 1174 collected on clean worktree @ gate SHA (1153 run + 21 other skips); KPI PDF test under `eval/retrieval/tests/`; +14 golden-vector tests when Option C vectors file present |
...
| T7 | No pytest detecting deletion-as-closure | **closed** — T12 `test_close_debt_preserves_committed_id_set` |
...
Recommended narrative-blind Phase 0 read **after** diff `3126c2b..1a2daeb`:
