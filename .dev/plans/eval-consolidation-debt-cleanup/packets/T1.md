# Executor packet — T1 · Fold enforcement across the `company_slug` parameter contract

**Plan:** `.dev/plans/eval-consolidation-debt-cleanup/plan.md` v1.0 (non-charter) · **Subtask:** T1 · **Log tier:** `architectural`
**Recommended executor:** Claude Sonnet 5 Thinking (or stronger) — see the *Recommended executor* field in §4 below for why this is not a Composer-eligible subtask.
...
| **Risks & mitigations** | *Over-broad guard rejects legitimate slugs* → mitigated by the idempotence assumption (A-5) plus `test_require_folded_company_slug_accepts_folded_slug` on `elder_care`, `clearsulting`, and `acme_corp`. *Guard placed behind the existing `strip()` so a padded slug slips through* → the guard must run on the **raw** parameter, and `test_load_gold_exclusions_rejects_unfolded_company_slug` should use `"Elder Care"` (spaces and capitals, the exact F-2-01 example). *Moving the fold in `main` changes the summary line* → §2.6 freezes the printed shape and `test_run_summary_line_names_company_slug_and_catalog` is the existing falsifier. *SQL-side UDF divergence* (C-5) → disproven by design, since kill criterion 5 forbids touching the fold's semantics. |

---

## 5.2 Load-bearing assumptions naming T1
...
- **C-4 · confirmed negative** — `(T1 and T2 share no import or call path, so they are genuinely parallel | trust_statement.py module imports :19-21 vs eval.retrieval.companies / gold.bootstrap / harness | none — negative finding recorded to justify the parallel edge | T1,T2)` — you may be running concurrently with T2 on `trust_statement.py`; do not touch that file.
- **C-5 · suspected** — `(eval/retrieval/scripts/apply_ops_ddl.sql:115 defines a warehouse-side canonical_company_slug UDF sharing the Python function's name | apply_ops_ddl.sql:115; trust_statement.py:932 fetch_company_domain_sql | if T1 changed the Python fold's semantics, the SQL UDF would silently diverge | T1)` — mitigated by kill criterion 5; **what would prove the coupling fired: any diff hunk inside `companies.py:18-35`.**

---

## Resolved inputs

**None — T1 is a source node.** No prior subtask output is required. Reference facts established at planning time and verified against the code, provided so you do not have to re-derive them:

**The fold authority (`eval/retrieval/companies.py:18-35`) is already correct — do not change it.**
...
**Fix locus 1 — `eval/retrieval/gold/bootstrap.py:278-288`.** The function accepts `company_slug` and only checks it is non-empty, then `strip()`s it. An unfolded display name such as `"Elder Care"` passes and falls through to `companies.get(slug)` → `None` → `return {}`, silently. The guard must run on the **raw** parameter, before `strip()`.
...
**Fix locus 2 — `eval/retrieval/harness.py:96-97`.** Zero validation; builds a path from whatever it is handed. This is the "library path" half of F-15.
...
**Fix locus 3 — `eval/retrieval/harness_cli.py:91-126`.** This is §8.4.3 T3 / O-2-2. `_run_harness_kwargs` folds only on the `else` branch (no `--gold-path`), so with `--gold-path` supplied the name is never folded until line 121 — which sits **after** `harness.run(...)` and outside any `try`. An unnormalizable `--company-name` therefore produces a traceback *after* a completed run with committed side effects. Note the existing `try`/`except (PreconditionError, ValueError)` at `:96-100` already returns exit 1 cleanly; the fold belongs inside it.
...
```python
    if args.command == "run":
        ...
        try:
            ...
            ...
        slug = canonical_company_slug(args.company_name)   # <-- :121, unguarded, post-run
        print(
...
**Existing test-coverage baseline (verified — these gaps are real, not already covered elsewhere):**
- `eval/retrieval/tests/test_gold_exclusions.py` exercises only well-formed slugs (`"elder_care"`, `"clearsulting"`, `"acme_corp"`); no test passes an unfolded display name.
- `eval/retrieval/tests/test_harness_fixture.py::test_default_gold_path_requires_slug` (`:418`) tests the *slug-required* precondition only, never fold-validity.
- `eval/retrieval/tests/test_harness_cli.py::test_run_explicit_gold_path_skips_company_slug_derivation` (`:95`) *documents* that the `--gold-path` branch skips folding — confirming the `:121` gap is intended-by-omission and untested.

**CLI test pattern to copy (`test_harness_cli.py:26-36`):** decorate with `@patch("eval.retrieval.harness_cli.EvalHarness")` and `@patch("eval.retrieval.harness_cli._build_store")`, then call `main([...])`. For your new test, an unnormalizable `--company-name` (e.g. `"!!!"`) plus `--gold-path` must give `exit_code == 1` **and** `mock_harness.run.assert_not_called()` — the not-called assertion is the actual content of the finding, since the defect is that the failure currently happens *after* the run.

**Positive-preservation target (M4 `decision-logs/T13.md`, landed contract):** "Require keyword-only `company_slug` on `load_gold_exclusions`; unknown slug → `{}`; malformed top-level → `PreconditionError`." A well-formed but unregistered slug like `"acme_corp"` equals its own fold, so it must still return `{}` and must not trip the new guard. This is a different code path from the unfolded-name path, and the two must not be collapsed (kill criterion 2).
