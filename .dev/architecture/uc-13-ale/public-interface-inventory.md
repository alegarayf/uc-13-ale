...
| `semantic_search_with_fallback` | `databricks/agents/shared/fallback.py` | function | Wraps `semantic_search`; retries without `file_name_filter` when results sparse | BMA, Legal, FTA `context_utils` | stable |
...
| `product_backlog.yaml` | `eval/program/product_backlog.yaml` | artifact | S2 product-signal ledger; optional `closed_at` + `closed_evidence_refs` on closed rows (no `status` field) | `test_product_backlog_schema.py`, playbook §3.1 | active |
...
| `evaluate_thresholds` | `eval/content/agreement.py` | function | C5 pinned threshold evaluation per surface class | `calibration.py`, hermetic tests | stable |
| `main` (calibration driver) | `eval/content/calibration.py` | function | `python -m eval.content.calibration` — judge-capability session against §8.7 samples | Operator M2 item 26a runs | active |
