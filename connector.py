...
def _normalize_folder_path(raw: str) -> str:
    """Convert sp_folder_path to a drive-relative path.

    The Graph API ``drive/root:{path}`` expects a path relative to the root of
    the "Shared Documents" document library — NOT a full server-relative URL.

    Acceptable secret values:
        /Nimble Gravity UC13                                          ← already correct
        /teams/RallydayPartnersExternal/Shared Documents/Nimble...   ← strip prefix

    Any prefix up to and including ``/Shared Documents`` is stripped so that
    either format works without requiring a secret update.
    """
...
def _env() -> tuple[str, str, str, str, str, str]:
    ...
    folder_path is always returned in drive-relative form (see _normalize_folder_path).
    """
    return (
        ...
        _normalize_folder_path(_get_secret("sp_folder_path")),
