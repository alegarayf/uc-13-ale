...
- **BMA extraction is a SINGLE call over the full unbounded context** (`business_model_agent.py`, `max_tokens=16_000`). A two-pass split — bounded/capped context (CIM → Tier 1 → other, 90K-char budget), two `max_tokens=8_000` calls over disjoint field groups (commercial vs organizational), merged into one dict — was proposed during the merge review of Hector's CIM-Rainmaker branch to avoid the ~120s serving-timeout risk on vision-heavy contexts, but was **explicitly rejected by Ale**: on Elder Care data both versions completed without crash or truncation, yet the two-pass version lost real diligence insight (`overlay_conflict_evidence`) and introduced noise (third-party tax preparers misclassified as `key_executives`), for no clear token-cost or latency win in the tested (non-vision) case. **Do not reintroduce a two-pass/multi-call split for BMA extraction without explicit owner approval.** If the timeout problem resurfaces, fix it another way — e.g. raise the timeout knob via `_get_llm_client`, or cap/prioritize context without splitting the extraction call — rather than re-adding the group-split pattern. Standing record: `.dev/merge-decisions.md`.
- If any **other** agent's single extraction grows past ~12K output, consider splitting it (the FTA sub-agent pattern is the precedent) rather than raising `max_tokens` — but **BMA is exempt**; see above.
