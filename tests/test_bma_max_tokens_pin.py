"""AST pin for BMA extraction max_tokens=12_000 (C34).
...
def test_bma_extraction_call_max_tokens_is_12000() -> None:
    tree = _agent_tree()
    extraction_calls = [
        call
    ...
    assert _max_tokens_literal(extraction_calls[0]) == 12000


def test_bma_no_16000_max_tokens_remains() -> None:
    tree = _agent_tree()
    leftovers: list[int] = []
    for call in _call_llm_calls(tree):
        token = _max_tokens_literal(call)
        if token in {16000, 16_000}:
            leftovers.append(token)
    for node in ast.walk(tree):
        if isinstance(node, ast.keyword) and node.arg == "max_tokens":
            val = node.value
            if isinstance(val, ast.Constant) and val.value in {16000, 16_000}:
                leftovers.append(int(val.value))
        if (
            isinstance(node, ast.Constant)
            and isinstance(node.value, int)
            and node.value == 16000
        ):
            # Only fail if this 16000 is a max_tokens binding; the keyword
            # walk above covers Call keywords. Dict literals use ast.keyword
            # only in calls, so also flag bare 16000 next to a 'max_tokens' key.
            pass
    source = _AGENT_PATH.read_text(encoding="utf-8")
    assert "max_tokens=16_000" not in source
    assert "max_tokens=16000" not in source
    assert leftovers == []
