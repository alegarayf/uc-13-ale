"""Re-run Company Profiler for GKF on serverless (eval-signal-foldback M6 T1).
...
Usage (from repo root):
  python eval/program/rerun_profiler_gkf.py
"""
...
CLUSTER_DRIVER = '''\
"""GKF Company Profiler re-run — cluster driver (M6 T1)."""
from __future__ import annotations
...
def _upload_driver(w: WorkspaceClient, user: str, source: str) -> str:
    driver_path = f"/Users/{user}/rerun_profiler_gkf_driver.py"
...
def main() -> int:
    ...
    if COMPANY_NAME != "GKF":
        raise RuntimeError(
            "sp_company_name / COMPANY_NAME must be the literal 'GKF' "
