"""Promote Clearsulting W2a golden-checklist scores (eval-signal-foldback M5 T5).
...
``record_e2e_linkage`` for Legal (agent-id ``legal``, never ``lca``). FTA is
already linked on this run_id and is not touched.

Usage (from repo root)::

    python eval/program/promote_w2a_clearsulting.py
"""
...
# Scores copied from each checklist's Summary line at T5 execution
# (T2 0b98176 / T3 cd0c972 / T4 4a65f8c). Do not invent replacements.
...
CLUSTER_DRIVER = '''\
"""Clearsulting W2a promotion — cluster driver (M5 T5)."""
from __future__ import annotations
...
mod_path = REPO_ROOT / "eval" / "program" / "promote_w2a_clearsulting.py"
spec = importlib.util.spec_from_file_location("promote_w2a_clearsulting", mod_path)
...
def run_promotion(spark: Any) -> None:
    """Cluster path: five gate-agent promotions, then Legal linkage."""
...
def _sync_eval_retrieval(w) -> int:
    ...
    for rel in SYNC_DIRS:
        ...
        for fp in local_root.rglob("*"):
            ...
            uploaded += 1
    script_ws = f"{root}/eval/program/promote_w2a_clearsulting.py"
...
def _upload_driver(w, user: str, source: str) -> str:
    driver_path = f"/Users/{user}/promote_w2a_clearsulting_driver.py"
...
def main() -> int:
    ...
    if "--no-sync" in sys.argv:
        script_ws = f"{repo_root}/eval/program/promote_w2a_clearsulting.py"
        _import_file(w, script_ws, Path(__file__).resolve())
