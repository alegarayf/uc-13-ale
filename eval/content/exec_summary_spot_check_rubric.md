...
| Business Overview | exec.claim.002 | The company generates revenue by billing private-pay clients for caregiver hours delivered. |
| Business Overview | exec.claim.003 | Elder Care had approximately 352 active clients as of Q2 2025. |
| Business Overview | exec.claim.004 | Elder Care had 2,123 registered caregivers across markets including NYC, Long Island, Westchester, NJ, MA, and CT as of Q2 2025. |
| Business Overview | exec.claim.005 | The platform completed at least two transactions — Guided Living and Unicity — within the recent historical window. |
| Business Overview | exec.claim.006 | Elder Care is currently ramping a Connecticut de novo location. |
| Financial Picture | exec.claim.007 | Pro Forma Adjusted Revenue reached $46.4M on a TTM Aug-24 basis per the Historical P&L Summary workstream. |
| Financial Picture | exec.claim.008 | Revenue grew from $8,955K in 2020A to $46,423K TTM Aug-24. |
| Financial Picture | exec.claim.009 | Pro Forma Adjusted EBITDA was $9,239K representing a 19.9% margin on a pro forma basis. |
| Financial Picture | exec.claim.010 | Reported (pre-addback) EBITDA margin is approximately 7.9%, below the ~10% healthcare services threshold. |
| Financial Picture | exec.claim.011 | The addback schedule contains 17 discrete Tier 4 items totaling approximately $7.3M in gross adjustments. |
| Financial Picture | exec.claim.012 | Gross addbacks represent approximately 247% of reported EBITDA. |
| Financial Picture | exec.claim.013 | Every addback item has been classified Tier 4 because no supporting documents are referenced in the addback schedule within the VDR. |
| Financial Picture | exec.claim.014 | Run-rate executive compensation addback is $2,490K. |
| Financial Picture | exec.claim.015 | Unicity pre-acquisition results addback is $1,077K. |
| Financial Picture | exec.claim.016 | Unicity synergies addback is $909K. |
| Financial Picture | exec.claim.017 | The reconciliation workstream flagged one mismatch and was unable to verify nine items. |
| Financial Picture | exec.claim.018 | The forecast projects revenue growing from $47.2M (2024E) to $197.8M (2029P) — a 4.2x increase. |
| Financial Picture | exec.claim.019 | The Forecast workstream rates the five-year revenue trajectory Red given aggressive assumptions. |
| Top Risks | exec.claim.020 | The company received adverse NYSDOH survey citations in May 2023 related to HCR Profile maintenance, background-check consent procedures, and Home Care Registry credentialing timelines. |
| Top Risks | exec.claim.021 | Written DOH confirmation of closure of the May 2023 NYSDOH citations has not been confirmed. |
| Top Risks | exec.claim.022 | Verification that no agreements exist with OIG/SAM-excluded individuals or entities has not been cleared. |
| Top Risks | exec.claim.023 | Outside counsel was retained as recently as April 30, 2025 to pursue collection of patient receivables. |
| Top Risks | exec.claim.024 | The Manhattan office lease contains an anti-assignment covenant that may require landlord consent as a closing condition. |
| Confidence | exec.claim.025 | Overall analytical confidence across all seven workstreams is rated Medium. |
| Confidence | exec.claim.026 | Five of seven workstreams carry at least one Red or Yellow rating on a material dimension. |
| Business Overview | exec.claim.027 | Elder Care operates zero Medicare-certified or Medicaid-reimbursed locations. |
| Financial Picture | exec.claim.028 | The Quality of Earnings workstream validated every Tier 4 addback against audited standalone financials. |
| Business Overview | exec.claim.029 | Elder Care provides home health aide, live-in, and nursing services to elderly patients. |
| Business Overview | exec.claim.030 | The platform has grown both organically and through acquisition. |
| Financial Picture | exec.claim.031 | The reported EBITDA base is roughly $7.7M. |
| Financial Picture | exec.claim.032 | Cash-to-accrual revenue adjustment addback is $665K. |
| Financial Picture | exec.claim.033 | Guided Living ramp-up adjustment addback is $430K. |
| Financial Picture | exec.claim.034 | Each of the five largest individual addbacks individually exceeds 5% of reported EBITDA. |
| Financial Picture | exec.claim.035 | The five largest addbacks include forward-looking pro forma synergies and pre-close acquisition earnings that have not been validated against audited standalone financials or demonstrated post-close performance. |
| Financial Picture | exec.claim.036 | If even a portion of these addbacks are rejected in a buyer QofE, the defensible EBITDA base could approach zero or turn negative, fundamentally altering valuation and leverage capacity. |
| Financial Picture | exec.claim.037 | Forecast revenue growth is driven by organic growth, the Connecticut de novo ramp, and additional expansion. |
| Top Risks | exec.claim.038 | The diligence process has surfaced several critical and material issues across workstreams. |
| Top Risks | exec.claim.039 | Corrective action plans and the absence of ongoing monitoring obligations for the NYSDOH citations have not been confirmed. |
| Top Risks | exec.claim.040 | Potential False Claims Act exposure from OIG/SAM exclusion screening has not been cleared. |
| Top Risks | exec.claim.041 | The addback stack's composition — including speculative forward synergies and unaudited pre-close acquisition earnings — represents the single most consequential open item for price and structure. |
| Top Risks | exec.claim.042 | Patient receivables collection activity raises questions about billing integrity and working capital quality. |
| Top Risks | exec.claim.043 | Material customer contracts contain termination-for-convenience provisions whose interaction with a change-of-control has not been confirmed. |
| Top Risks | exec.claim.044 | Two identified clients show significant multi-year billing declines whose treatment in the run-rate revenue base has not been clarified. |
| Top Risks | exec.claim.045 | Unusual indemnity provisions appear across multiple contracts including the Unicity Asset Purchase Agreement. |
| Confidence | exec.claim.046 | Five of seven workstreams carry a Red rating (Financial Trends, KPI, Legal Contracts, Quality of Earnings, and Forecast). |
| Confidence | exec.claim.047 | Two workstreams are rated Yellow (Business Model, Customer Quality). |
| Confidence | exec.claim.048 | Open item 1: seller-provided audited or third-party-verified support is required for each of the 17 Tier 4 addbacks, with particular focus on the five items individually exceeding 5% of EBITDA. |
| Confidence | exec.claim.049 | Open item 2: written DOH confirmation of regulatory citation closure and a completed OIG/SAM exclusion screening for all employees, contractors, and vendors are required. |
| Confidence | exec.claim.050 | Open item 3: quantification and aging of receivables in active legal collection are required. |
| Confidence | exec.claim.051 | Open item 4: confirmation is required of whether material customer contracts contain change-of-control triggers. |
| Confidence | exec.claim.052 | Open item 5: location-level P&L and operational KPIs for Guided Living, Unicity, ECHC, and the Connecticut de novo are required to validate the entity-level EBITDA build-up. |
| Confidence | exec.claim.053 | The Legal Contracts workstream should engage outside counsel to confirm whether the Manhattan lease anti-assignment provision applies to the contemplated transaction structure. |
...
**Listing evidence:** T2 item-23 probe session enumerated **276** numeric claims (`numeric_claim_count: 276`). Committed manifest: `eval/content/fta_numeric_rubric_claims.json` (extracted from this rubric via `eval/content/extract_rubric_manifests.py`; operator probe record under `.dev/plans/.../t2_fta_probe_report.json`). Claim IDs below use stable `fta.claim.NNN` keys aligned to that probe ordering.
