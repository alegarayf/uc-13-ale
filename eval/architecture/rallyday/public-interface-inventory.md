...
| Symbol | Module | Kind | Signature summary | Consumed by | Stability |
|--------|--------|------|-------------------|-------------|-----------|
| `semantic_search` | `databricks/agents/shared/retrieval.py` | function | VS query + merge-rank (`sim × tier_weight`, `merge_rank_mode` kwarg for ablation) + optional VS metadata filter pushdown (`vs_metadata_filters=False` default — item 29 not activated M-PHV4) + keyword fallback → `RouteResult` | BMA/CQA/KPI/Legal/QoE agents; FTA via `context_utils` thin delegator; `EvalHarness.dispatch_retrieval` | active |
...
| `FinancialTrendsAgent.run` | `databricks/agents/workstreams/financial_trends_agent.py` | method | Orchestrates 3 parallel sub-agents via `ThreadPoolExecutor`; `(company_name, spark, llm_endpoint, …, catalog)` — no `retrieval_mode` param (M-PHV4 T2) | `test_pipeline.ipynb` Cell 12 | active |
