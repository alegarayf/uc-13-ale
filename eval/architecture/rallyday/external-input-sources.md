...
```
Source:               VDR UI submissions (companies_vdr_history)
Format:               Databricks Delta table row triggering run_vdr_pipeline job
Parser:               run_vdr_pipeline.py reads row by id; no document parsing at trigger
Trust level:          partially trusted — operator-submitted company name and source_data_location
Surfaces extracted:   processing_status transitions; output paths to VDR volume (full_report.docx, executive_summary.docx)
Surfaces NOT extracted: Raw VDR source files (ingestion uses SharePoint path separately)
...
```
Source:               Rainmaker reference PDF (operator-provided, image-only)
Format:               PDF (structural validation manual — no automated text diff)
Parser:               Operator visual comparison against Rev3 rendered H2 order
Trust level:          trusted for layout fidelity target
Surfaces extracted:   Section order vocabulary for Rev2/Rev3 acceptance decisions
Surfaces NOT extracted: Automated PDF text extraction
Volume:               Per validation run (Elder Care T3/T6/T24)
Sensitivity:          Stakeholder presentation benchmark
Owner module:         operator validation workflow (.dev/t*_elder_care_validation_decision.md)
```
