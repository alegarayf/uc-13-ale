"""Hermetic pins for eval/program/promote_w2c_spg.py — M7 T6."""
...
_SCRIPT = _REPO_ROOT / "eval" / "program" / "promote_w2c_spg.py"

_GATE_IDS = ("bma", "cqa", "kpi", "qoe", "profiler")
_CHECKLIST_SCORE_PATHS = {
    "bma": _REPO_ROOT / "eval" / "BMA" / "golden_checklist_spg.md",
    "cqa": _REPO_ROOT / "eval" / "CQA" / "golden_checklist_spg.md",
    "kpi": _REPO_ROOT / "eval" / "KPI" / "golden_checklist_spg.md",
    "qoe": _REPO_ROOT / "eval" / "QOE" / "golden_checklist_spg.md",
    "profiler": _REPO_ROOT / "eval" / "PROFILER" / "golden_checklist_spg.md",
    "legal": _REPO_ROOT / "eval" / "LCA" / "golden_checklist_spg.md",
}
_FTA_CHECKLIST = _REPO_ROOT / "eval" / "FTA" / "golden_checklist_spg.md"
...
def test_company_name_is_spg_uppercase() -> None:
    assert _assign_str(_tree(), "COMPANY_NAME") == "SPG"
    source = _SCRIPT.read_text(encoding="utf-8")
    assert 'COMPANY_NAME = "spg"' not in source
...
def test_qoe_candidate_total_is_five() -> None:
    qoe = next(c for c in _gate_calls(_tree()) if c["e2e_agent_id"] == "qoe")
    assert qoe["candidate_total"] == 5
    assert qoe["candidate_score"] == 3
...
def test_fta_linkage_score_is_literal_integer_eight() -> None:
    """Point-literal pin: FTA write is 8, not 8.5 / 9 / 14 / float."""
    tree = _tree()
    ...
    assert "8.5" not in source
    assert "e2e_checklist_score=8" in source or "e2e_checklist_score = 8" in source


def test_fta_checklist_weighted_observable_stays_eight_point_five() -> None:
    """Producer-versus-data: checklist 8.5/18 is distinct from linkage 8/18."""
    text = _FTA_CHECKLIST.read_text(encoding="utf-8")
    summary = next(line for line in text.splitlines() if line.startswith("**Summary:**"))
    assert "8.5/18" in summary
    assert "**8/18**" not in summary
...
def test_run_id_matches_spg_pipeline_manifest() -> None:
    assert _assign_str(_tree(), "RUN_ID") == "445878b36e06407385b9498dcab265c7"
