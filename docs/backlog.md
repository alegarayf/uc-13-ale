...
> Context: see [`session_log.md`](./session_log.md).
> Branch: `feature/uc13-after-merge-job-test`.
> **Note:** this file lives in `docs/` (gitignored — personal, local-only). Updated by the `update-backlog` skill.

## 🟣 Product roadmap — next few days (Hector's priorities)

Three product-facing initiatives. **Reordered 2026-08-03** (see decision note below) — #2 now comes first, using the existing prose Rev3 one-pager as the preview format; #1 (visual/slide layer) demoted to a later enhancement of #2's output rather than a hard dependency.

**Branch:** `feature/vdr-cim-preview-flow` (cut from `feature/uc13-after-merge-job-test` @ `d0d54ef`) — created for this work; planning + execution to start in a future session.

### 1. CIM-first staged flow — cheap preview before full pipeline (start here first)
Architectural change to the execution flow, gated by CIM presence:
- **If a CIM-like file exists** in the data room: run a cheap pre-step that generates an **executive preview** — for now, using the **existing prose Rev3 one-pager bridge** (`agents/exec_summary/pipeline_entry.py`, `bundle_builder.py`) as the render target, *not* the visual/slide format from item #2 below (which doesn't exist yet — see decision note) — from just the CIM (+ a small set of user-provided key files in a VDR SharePoint folder). *Not* the full Phase 1-5 pipeline.
...
- No longer blocked on item #2 (decision below) — can start with the prose format now.

### 2. Rainmaker format layer — visual executive summary (PDF/slide-style) — demoted, do after #1
...
- **Decision note (2026-08-03):** confirmed this does *not* exist yet — "Rainmaker" in the current code (`pipeline_entry.py`, `run_vdr_pipeline.py`) only names the existing **prose** Rev3 one-pager bridge, not a visual/slide format. Hector chose to build #1 (CIM-first preview) against the prose format now rather than block on this; revisit as an enhancement once #1's gate/flow is in place — the preview step can swap render targets later without changing the gate logic.
- Likely lands near `agents/exec_summary/` (the existing Rev3 one-pager bridge / `bundle_builder.py` / `pipeline_entry.py`).
...
- **Reconcile BMA extraction approach with Ale — NEW, from PR #3 review.** Ale reverted Hector's two-pass split (`39bb2b8`) back to a single-call Sonnet + `max_tokens=16_000` path on `merge-ale-base-hector-incoming-results`, citing his own Chip B e2e validation (SPG, 71k chunks, `uc13_ale`). Hector restored the two-pass split on this branch (`60d8000`) instead, since it's what's validated against Rallyday's `uc13` ~120s serving-timeout ceiling (Clearsulting run 31, GKF run 36). Two competing, independently-validated fixes now live on two branches — not reconciled. See [`session_log.md` §6](./session_log.md) for full detail. Needs a decision before either branch reaches a shared/prod target.
...
- **PR of `feature/uc13-after-merge-job-test` → `dev`/`main` when stable — partially progressed.** PR #3 merged this branch into `merge-ale-base-hector-incoming-results` (an intermediate branch, not `dev`/`main`); Hector then pulled that back into this branch. The actual promotion to `dev`/`main` is still open and now additionally blocked on the BMA reconciliation above.
- **Repo hygiene: root-level `.md` / temp files — decision reversed, not a backlog item anymore.** Ale attempted this cleanup (`433e05f`, untracked 18 files + `.gitignore`); Hector explicitly restored them as tracked on this branch (`6f55ce1`) because they're his working notes. No further action expected here unless Hector changes his mind.
