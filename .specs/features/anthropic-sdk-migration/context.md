...
| D-4 | Egress de red desde jobs serverless hacia `api.anthropic.com` | **Desconocido — se verifica.** Smoke test como gate bloqueante antes de tocar código de producción. → **RESUELTO 2026-09-01: hay egress.** | Phase 0 fue el smoke test. Salió `OK` en serverless para ambos model IDs, así que no hubo que escalar a infraestructura y la contingencia B no se activó. Evidencia: `signoffs/ASDK-13-egress-gate.md`. |
