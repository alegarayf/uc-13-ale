...
- [x] Contador de degradaciones y resumen de tokens de la corrida registrados — **tokens** dentro de ±6% del baseline en los tres (−2.7% / +5.4% / +1.0%). **Degradaciones: 0**, cerrado el 2026-09-02 por vía API tras descartarla inicialmente. El contador in-process no se persiste, pero el atributo de span **sí**: `llm_client._safe_set_span_attributes` graba `llm.fallback_used` y `llm.backend` en cada llamada, y MLflow los persiste. Consultando el experimento del notebook (`250e0a2c79064562a950a6cddb00afd2`) en la ventana de las tres corridas: **266 spans de gateway, los 266 con `llm.fallback_used=False` y `llm.backend=anthropic`**. Evidencia más fuerte que la lectura visual del stdout que se había planteado: es completa (toda llamada, no un barrido a ojo) y consultable.
  - *Corrección de método*: se dio por no observable desde la API porque `jobs/get-run-output` devuelve `logs` vacío en tareas serverless, y se buscó el dato en el canal equivocado — stdout del driver. El diseño de tracing de T11 ya lo estaba persistiendo en un canal estructurado. La lección es mirar qué **ya se persiste** antes de declarar algo no observable.
